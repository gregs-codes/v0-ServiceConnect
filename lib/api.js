...
async function fetchAPI(endpoint, options = {}) {
  ...
  try {
    ...
    try {
      ...
      console.error(`Error reading response text from ${endpoint}:`, e)
    ...
    if (!responseText || responseText.trim() === "") {
      ...
      // Return a default response for empty but successful responses
    ...
    try {
      ...
      console.error(`Invalid JSON response from ${endpoint}:`, responseText)
      throw new Error(`Invalid JSON response from ${endpoint}: ${responseText.substring(0, 100)}...`)
    }
